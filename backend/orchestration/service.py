...
class JobService:
    ...
    def list_jobs(self, project_id: str | None = None, limit: int = 50, offset: int = 0) -> JobListResponse:
        ...
        return JobListResponse(items=[_to_summary(r) for r in rows])
    ...
    def resume(self, job_id: str) -> JobDetail:
        ...
        if str(job.get("desired_state") or "").startswith("rerun_node_complete:"):
            raise StageExecutionConflict(
                "isolated node rerun completed; choose a formal stage execution command before continuing"
            )
    ...
    def execute_from_stage(self, job_id: str, request: StageExecutionRequest) -> JobDetail:
        ...
        with self.db.transaction(immediate=True) as conn:
            ...
            downstream = [
                ...
                and (not requested_shot or (row["shot_id"] or "") in {requested_shot, ""})
            ]
            ...
            downstream_status = StepStatus.PENDING.value if request.mode == "continue" else StepStatus.INVALIDATED.value
            ...
            desired_state = "running" if request.mode == "continue" else f"pause_after_step:{target['id']}"
            conn.execute(
                """UPDATE jobs
    ...
    def _find_failed_step_id(self, job_id: str) -> str | None:
        for s in self.repo.get_job_steps(job_id):
    ...
    def _stage_plan_for_job(self, data: JobCreate, settings: JobSettings) -> list[dict[str, str]]:
        ...
        return build_production_stages(shot_ids=shot_ids) if shot_ids else build_production_stages(max_shots=settings.options.max_shots)
    ...
    def _build_detail(self, job_id: str) -> JobDetail:
        ...
        return JobDetail(
            id=job["id"], project_id=project_id, status=job["status"], mode=job["mode"],
            desired_state=job["desired_state"], current_stage=job["current_stage"],
            current_shot=job["current_shot"], progress=job["progress"], message=job["message"],
            final_video=job["final_video"], created_at=job["created_at"], updated_at=job["updated_at"],
            finished_at=job.get("finished_at"), steps=[StepInfo(**s) for s in steps],
...
def _to_summary(row: dict) -> JobSummary:
    return JobSummary(
        ...
        desired_state=row["desired_state"], current_stage=row["current_stage"],
        current_shot=row["current_shot"], progress=row["progress"], message=row["message"],
        final_video=row["final_video"], created_at=row["created_at"], updated_at=row["updated_at"],
