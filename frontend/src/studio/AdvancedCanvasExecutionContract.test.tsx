...
describe("AdvancedCanvasWorkspace v0.9 execution contract", () => {
  afterEach(cleanup);

  it("does not claim that a node ran when the canvas has no stage-start backend contract", () => {
    render(<AdvancedCanvasWorkspace />);

    fireEvent.click(screen.getByRole("button", { name: "运行选中节点" }));

    expect(screen.queryByText(/运行选中节点：TI2V视频生成/)).not.toBeInTheDocument();
    expect(screen.getByText(/高级画布暂不直接提交单节点任务/)).toBeInTheDocument();
